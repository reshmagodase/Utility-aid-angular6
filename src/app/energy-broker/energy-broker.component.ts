...
export class EnergyBrokerComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.adForm.valid) {
      console.log("form value", this.adForm.value);
      this.servicecalls.postServerRes("sendGoogleAdContact", { name: this.adForm.value.name, cvemail: this.adForm.value.cvemail, contactNo: this.adForm.value.contactNo, message: this.adForm.value.message }).pipe(
  ...
  onLeadFormSubmit() {
    ...
    if (this.leadForm.valid) {
      console.log("form value", this.leadForm.value);
      this.servicecalls.postServerRes("sendLeadContact", { name: this.leadForm.value.name, cvemail: this.leadForm.value.cvemail, contactNo: this.leadForm.value.contactNo, company: this.leadForm.value.company }).pipe(
        tap((resp: any) => console.log('heaeder', resp.headers.get('ReturnStatus')))
      ).subscribe((data: any) => {
        // console.log(data.headers.get('X-Token'));
        console.log(data);
  ...
  onLead1Submit() {
    ...
    if (this.leadForm2.valid) {
      ...
      this.servicecalls.postServerRes("sendGoogleAdContact", { name: this.leadForm2.value.name, cvemail: this.leadForm2.value.cvemail, contactNo: this.leadForm2.value.contactNo, message: this.leadForm2.value.message }).pipe(
        tap((resp: any) => console.log('heaeder', resp.headers.get('ReturnStatus')))
      ).subscribe((data: any) => {
        // console.log(data.headers.get('X-Token'));
        console.log(data);
